...
export default function EyeCareGlobal() {
  ...
  useEffect(() => {
    ...
    if (!saved) {
      ...
      else if (browserLang.startsWith('ru')) setLanguage('ru');
      else if (browserLang.startsWith('ar')) setLanguage('ar');
      else if (browserLang.startsWith('hi')) setLanguage('hi');
  ...
  return (
    <>
      ...
      <div className={`min-h-screen ${
        ...
        {/* Top Banner with Sponsor and Language */}
        <div className="fixed top-0 left-0 right-0 h-16 bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm flex items-center justify-between px-6 border-b border-gray-200/30 z-40">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="px-4 py-2 bg-white rounded-lg shadow-sm border border-gray-200 flex items-center gap-2 hover:bg-gray-50 transition-colors"
            >
              <span className="text-lg">{languages.find(l => l.code === language)?.flag}</span>
              <span className="text-sm font-medium text-gray-700">{languages.find(l => l.code === language)?.name}</span>
